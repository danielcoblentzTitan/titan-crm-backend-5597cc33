...
import { WeatherDelayDialog } from "./WeatherDelayDialog";
...
export function MasterScheduleView({ selectedDate = new Date() }: MasterScheduleViewProps) {
  const [currentMonth, setCurrentMonth] = useState(startOfMonth(selectedDate));
  const [projectSchedules, setProjectSchedules] = useState<ProjectScheduleData[]>([]);
  ...
  // Load all project schedules and weather days
  const loadProjectSchedules = async () => {
    ...
    try {
      ...
      const { data: schedules, error } = await supabase
        ...
        `)
        .not('projects.status', 'eq', 'Completed')
      ...
      const processedSchedules: ProjectScheduleData[] = schedules?.map(schedule => {
      ...
      setProjectSchedules(processedSchedules);
    } catch (error) {
  ...
  }, [currentMonth]); // Reload when month changes to get weather data for new month range

  // Get trade segments for a specific week for a specific project
  const getTradeSegmentsForWeek = (week: Date[], projectSchedule: ProjectScheduleData) => {
  ...
  return (
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Badge variant="secondary">{projectSchedules.length} Active Projects</Badge>
            <Button
              variant="outline"
              size="sm"
      ...
      <CardContent className="space-y-6">
        ...
        {/* Calendar Grid with stacked projects */}
          ...
          <div key={weekIndex} className="relative">
            ...
            <div className="grid grid-cols-5 gap-1">
              {week.map((day, dayIndex) => {
                ...
                const allTradesOnDay = projectSchedules.flatMap(projectSchedule => 
                  getTradeSegmentsForWeek(week, projectSchedule)
                ...
                return (
                  <div
                    key={day.toISOString()}
                    className={cn(
                      "p-1 border border-border bg-background rounded relative group",
                    ...
                    <div className="text-xs font-medium flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <Button
                          ...
                          onClick={() => handleAddWeatherDelay(day)}
                          title="Add weather delay for this date"
                        >
            ...
            {/* Trade Bar Overlays for this week - projects stacked vertically */}
            <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
              <div className="grid grid-cols-5 gap-1 h-full">
                {week.map((day, dayIndex) => {
                  return (
                    <div key={dayIndex} className="relative">
                      {projectSchedules.map((projectSchedule, projectIndex) => {
                        const projectTrades = getTradeSegmentsForWeek(week, projectSchedule)
                          .filter(segment => segment.startIndex === dayIndex);
                        
                        if (projectTrades.length === 0) return null;
                        
                        // Calculate project base offset - each project gets its own stack level
                        const projectBaseOffset = projectIndex * 25; // 25px spacing between projects
                        
                        return projectTrades.map((segment, tradeIndex) => (
                          <div
                            key={`${projectSchedule.project_id}-${segment.trade.name}-${weekIndex}-${tradeIndex}`}
                            className="absolute text-xs p-1 rounded text-white font-medium flex items-center pointer-events-auto"
                            style={{
                              backgroundColor: segment.trade.color,
                              left: '4px',
                              right: segment.length > 1 ? `calc(-${(segment.length - 1) * 100}% - ${(segment.length - 1) * 4}px - 4px)` : '4px',
                              top: `${20 + projectBaseOffset + (tradeIndex * 20)}px`,
                              height: '18px',
                              zIndex: 10
                            }}
                            title={`${projectSchedule.customer_name.split(' ').pop()} - ${segment.trade.name} - ${format(segment.trade.startDate, 'MMM d')} to ${format(segment.trade.endDate, 'MMM d')}`}
                          >
                            {segment.isFirstSegment && (
                               <span className="truncate text-xs">
                                {projectSchedule.customer_name.split(' ').pop()} - {segment.trade.name}
                              </span>
                            )}
                          </div>
                        ));
        ...
        {projectSchedules.length === 0 && (
          <div className="text-center py-8 text-muted-foreground">
            No active project schedules found
          </div>
        )}
      </CardContent>
