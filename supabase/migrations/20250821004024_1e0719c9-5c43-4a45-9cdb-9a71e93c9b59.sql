...
DROP POLICY IF EXISTS "Builders can view all users" ON public.users;
DROP POLICY IF EXISTS "Users can view their own profile" ON public.users;
DROP POLICY IF EXISTS "Builders can view all team members" ON public.team_members;
DROP POLICY IF EXISTS "Builders can view all projects_new" ON public.projects_new;
DROP POLICY IF EXISTS "PM can view assigned projects_new" ON public.projects_new;
...
-- Handle phases_new table if it exists
DO $$ 
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'phases_new') THEN
    ...
    EXECUTE 'DROP POLICY IF EXISTS "Builders can view all phases_new" ON public.phases_new';
