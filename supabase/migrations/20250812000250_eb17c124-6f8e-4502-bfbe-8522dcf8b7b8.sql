-- Check if foreign key references the wrong table and fix it, then add sample data
-- First, let's see what foreign key constraint exists and fix it

-- Drop the existing foreign key constraint if it exists and recreate with correct table reference
DO $$ 
BEGIN
...
-- Now insert sample project phases using the correct project IDs
