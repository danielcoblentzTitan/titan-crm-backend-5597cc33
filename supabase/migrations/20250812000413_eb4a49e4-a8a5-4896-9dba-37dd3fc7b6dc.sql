-- Clean up existing data and fix foreign keys, then add sample data

-- First delete any existing milestones that reference non-existent projects
DELETE FROM project_milestones 
WHERE project_id NOT IN (SELECT id FROM projects_new);

-- Now drop and recreate the foreign key constraints correctly
DO $$ 
BEGIN
...
-- Now insert sample project phases
